...
int delay[] = {
  ...
  //r1: shift register of G1
  //r2: shift register of G2
  int c1, c2;
  int i,j;
  ...
  for (i=0; i<CA_SEQ_LEN; i++)
  {
    ...
    //x(2) XOR x(3) XOR x(6) XOR x(8) XOR x(9) XOR x(10)
    ...
    r1[0] = c1;
    //intut for r1 after shifting is c1
    r2[0] = c2;
    //input for r2 after shifting is c2
  }
...
int checkSatVisibility(ephem_t eph, gpstime_t g, double *xyz, double elvMask, double *azel)
{
  ...
  if (eph.vflg != 1)
    return (-1); // Invalid
  //convert to lat long heigth
  xyz2llh(xyz,llh);
  //calculate intermediate matrix to convert from llh to neu
  ltcmat(llh, tmat);

  //calculate coordinates of satellite from ephemeris
  satpos(eph, g, pos, vel, clk);
  //subtract vector between receiver and satellite
  subVect(los, pos, xyz);
  //calculate North East Up geometry from ECEF
  ecef2neu(los, tmat, neu);
  //calculate azimuth and elevation
  neu2azel(azel, neu);
  //if elevation < 0.0, This means satellite doesn't appear in the sky
...
int main(int argc, char *argv[])
{
  ...
  while ((result=getopt(argc,argv,"e:u:g:k:l:o:s:b:T:t:d:v:i:m"))!=-1)
  {
    switch (result)
    {
      ...
      case 'k':
        ...
        if (mode != 1)
        {
          printf("ERROR: enable online mode first: '-m 1'\n");
          exit(1);
        }
        else
        {
  ...
  if ((mode == 1) && (strlen(server) > 0))
  {
    ...
    self_sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (self_sock == -1)
    {
      printf ("Error 1!: %s\n", strerror(errno));
      close(self_sock);
    ...
    {
      printf ("Socket is created success\n");
    }
    //If socket is created success, connect to server
    printf ("Waiting to connect to server\n");
    ret = connect(self_sock,(struct sockaddr*)&Saddr,sizeof(Saddr));
    if (ret == -1)
    {
      printf ("Error 2!: %s\n", strerror(errno));
      close(self_sock);
...
//--> If current time is greater than tmax -> using the latest eph set
  ...
  //Open thread if using online mode
  //if ((mode == 1) && (strlen(server) > 0))
    //pthread_create(&recv_thread, NULL, &recvfromser, &self_sock);
  for (iumd=1; iumd<numd; iumd++)
  {
    ...
    if (interactive)
    {
      ...
      if ((key_direction!=UNDEF)/*&&(direction==key_direction)*/)
      {
        ...
        //printf ("Key_direction: %d, direction: %d\n",key_direction, direction);
      }
      else
      {
        ...
        //printf ("------------Key_direction: %c, direction: %c\n",key_direction, direction);
      }
      ...
      if ((direction!=UNDEF)&&(velocity>=0.0))
      {
        ...
        //printf ("\nX= %lf, Y= %lf, Z= %lf, Ver= %lf, Direction = %d\n", xyz[iumd][0], xyz[iumd][1], xyz[iumd][2],velocity, direction);
      }
